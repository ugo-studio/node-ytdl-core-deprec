...
exports.exposedMiniget = (url, options = {}, requestOptionsOverwrite) => {
  let requestOptions = requestOptionsOverwrite || options.requestOptions;
  // intercept request
  if (typeof options.interceptRequest === "function") {
    const intercepted = options.interceptRequest(url, requestOptions);
    url = intercepted.url;
    requestOptions = intercepted.req;
  }
  // make request
